...
app.post("/api/auth/register", async (req, res) => {
  ...
  if (await db.findUserByUsername(name)) return res.status(409).json({ error: "Username already taken" });
...
app.post("/api/auth/login", async (req, res) => {
  const { username, password } = req.body || {};
  const user = await db.findUserByUsername((username || "").toString().trim());
...
app.get("/api/tasks", authRequired, async (req, res) => {
  ...
  res.json({ tasks: await db.listTasks(req.user.id, date, status) });
});
...
app.post("/api/tasks", authRequired, async (req, res) => {
  ...
  const task = await db.createTask(req.user.id, text, taskDate, (description || "").toString());
...
app.patch("/api/tasks/:id", authRequired, async (req, res) => {
  ...
  const existing = await db.getTask(id, req.user.id);
  ...
  res.json({ task: await db.updateTask(id, req.user.id, fields) });
});

app.delete("/api/tasks/:id", authRequired, async (req, res) => {
  const id = Number(req.params.id);
  if (!(await db.deleteTask(id, req.user.id))) return res.status(404).json({ error: "Task not found" });
  res.json({ ok: true });
});

// ---------- Share / WhatsApp ----------
app.post("/api/share", authRequired, async (req, res) => {
  const token = await db.ensureShareToken(req.user.id);
...
app.get("/api/share/:token", async (req, res) => {
  const user = await db.getUserByShareToken(req.params.token);
  if (!user) return res.status(404).json({ error: "Invalid share link" });
  const tasks = await db.listTasks(user.id, null, "all");
